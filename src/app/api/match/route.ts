...
async function loadBreeds(): Promise<Breed[]> {
  ...
  // Public folder contents need special handling in Vercel standalone mode
  const possiblePaths = [
    path.join(process.cwd(), 'public', 'data', 'breeds.csv'),
    path.join(process.cwd(), '.next', 'standalone', 'public', 'data', 'breeds.csv'),
